...
**Tech Stack:** Vite 5 (existing), React 18, Three.js, `@vitejs/plugin-react`. No test framework exists in this repo (no jest/vitest, no `test` script) — verification is done by running the dev server and driving the page in a real browser (Playwright MCP tools if available, otherwise manual), matching how the existing DLVO/vdW/Derjaguin applets are verified in this project.
...
- [ ] **Step 4: Restart the dev server and manually verify full interactivity**

Run: `npm run dev` (restart if it was already running, so the new `App.jsx` is picked up — Vite HMR should also pick it up live).

Using a browser (Playwright MCP tools if available):
- Navigate to `http://localhost:5173/nano-explorer/src/applets/crystallography/`.
- Confirm the 3D unit cell renders (WebGL canvas visible, not blank/black).
- Confirm the task card shows a Miller index like `(001)` and the problem chip row is visible.
- Drag-rotate the cell (click-drag on empty canvas space) and confirm the view rotates.
- Drag one of the steel-colored intercept handle spheres along an axis and confirm the readout number and shaded plane update live.
- Click "Check" and confirm feedback text appears (correct or not-matching).
- Click "Reveal answer" and confirm the green target plane and comparison table appear.
- Click "Next plane" and confirm the problem advances and the scene resets.
- Check the browser console for zero errors during all of the above.

Expected: all interactions work exactly as they did in the original standalone component; no console errors.
...
With `npm run dev` running, navigate to `http://localhost:5173/nano-explorer/`.

Confirm:
- A new card appears after the "Derjaguin Approximation" card and before "Brownian Diffusion", with the 💠 icon on a cream gradient banner, tag "Crystal Structure", title "Miller Indices", and the description text above.
- Clicking the card's "Open applet" link navigates to the crystallography applet page.
...
Using a browser:
- Navigate to `http://localhost:4173/nano-explorer/`, confirm the Miller Indices card is present.
- Click through to the crystallography applet, confirm the 3D scene renders and at least one drag interaction works, with zero console errors.
